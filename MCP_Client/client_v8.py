...
async def connect_to_mcp_server(
    ...
    mcp_server_params = StdioServerParameters(
        command=command,
        args=args if args is not None else [SERVER_SCRIPT], # args or [SERVER_SCRIPT],
        env=env,
    )
...
async def run_pipeline(release_payload: dict, DEBUG_PRINT_ALL_MESSAGE_DETAILS = False):
    async with AsyncExitStack() as stack:
        ...
        print(f"🔧 🐙 Tools available: {[t['name'] for t in github_anthropic_tools]}\n")

        # Build a dispatch map at setup time
        tool_session_map = {}
...
def test_github():
    ...
    async def _test_github():
        async with AsyncExitStack() as stack:
            ...
            tools = github_tools_result.tools
            print("Tools")
            print("-----------------------------------------------------------------")
            for idx, tool in enumerate(tools, start=1):
                print(f"{idx})  name: {tool.name} | description: {tool.description}")
            ...
            if OWNER and REPO:
                ...
                raw_text = list_commits.content[0].text
                commits_data = json.loads(raw_text)

                for no, commit in enumerate(commits_data, start=1):
                    print(f"{no}) {commit['commit']['author']['name']} | {commit['commit']['author']['date']} | {commit['commit']['message']}")
